function[]=q2b(image,n_var,flag)
    ...
    for c=1:10
        g=adding_noise(g);
        g=weiner_filter_withc(g,c);
        MSE(c)=MSE_cal(image,g);
        x(c)=c;
        close all;
    end
    figure;plot(x,MSE);title('Weiner Filter MSE with c values');xlabel('c values');ylabel('MSE');
...
%                 figure;imagesc(image-g);colormap gray;title('diff Image');colorbar;
    ...
    function[g3]=weiner_filter_withc(g,c)
       threshold=0.5;
        g3=g;
       G=fft2(g);
        %S=var(F(:)); %Svalue in withc is calculated for each values in
        mult_constant=var(F(:));
        %loop
        K=var(E(:));
        ...
        for i=1:s1
            for j=1:s2
                S=mult_constant/(power(i^2+j^2,c));
                num=(abs(H(i,j)))^2;
                den=H(i,j)*(num+K/S);
                if(H(i,j)>threshold)
                    W(i,j)=num/den;
